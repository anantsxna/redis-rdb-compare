...
public class CountQuery extends Query {
    ...
    public void execute() {
        try {
            ...
            //
            Set<String> setCombine = botSession.getTrieE().getChildren(key);
            ...
            for (String parentKey : setCombine) {
                int count = botSession.getTrieE().getCountForPrefix(parentKey);
                //                int countInB = botSession.getTrieD().getCountForPrefix(parentKey);
                sortedResult.add(new AbstractMap.SimpleEntry<>(count, parentKey));
            }
